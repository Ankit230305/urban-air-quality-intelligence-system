...
def parse_args():
    ...
    ap.add_argument(
        "--target", default="pm2_5", help="Column to detect anomalies on (default: pm2_5)"
    )
...
def main():
    ...
    if target not in df.columns:
        ...
        pd.DataFrame(columns=["datetime", target, "z_score", "is_anomaly"]).to_csv(
    ...
    roll_std = (
        df[target].rolling(window=window, min_periods=1, center=True).std(ddof=0).replace(0, np.nan)
    )
